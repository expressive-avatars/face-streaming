  ...
  init: function () {
    ...
    const onFace = ({ blendShapes, headOrientation }) => {
      if (blendShapes && headOrientation) {
        ...
        const negativeInfluence = (blendShapes.mouthFrownLeft + blendShapes.mouthFrownRight) * 0.5
        const positiveInfluence = (blendShapes.mouthSmileLeft + blendShapes.mouthSmileRight) * 0.5
        auraEl.setAttribute('aura', {
          size: Math.max(negativeInfluence, positiveInfluence),
          color: positiveInfluence > 0 ? '#f5f242' : '#c8dff7',
        })
