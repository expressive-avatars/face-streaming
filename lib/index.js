...
AFRAME.registerSystem("preview-self", {
  init: function() {
    ...
    const camera = new THREE.PerspectiveCamera(10, 1, 0.1, 1e3);
...
const widget = "_widget_12x08_7";
const row = "_row_12x08_20";
const btn = "_btn_12x08_26";
const primary = "_primary_12x08_39";
const btnCollapse = "_btnCollapse_12x08_49";
const collapsible = "_collapsible_12x08_56";
var style = {
  widget,
  row,
  btn,
...
link$1.href = "https://cdn.jsdelivr.net/npm/boxicons@2.1.1/css/boxicons.min.css";
...
AFRAME.registerSystem("facetracking-widget", {
  ...
  init: function() {
    ...
    const btnCollapse2 = h("button");
    btnCollapse2.classList.add(style.btnCollapse);
    btnCollapse2.addEventListener("click", () => this.toggleCollapse());
    ...
    const collapsible2 = h("div");
    collapsible2.classList.add(style.collapsible);
    const widget2 = h("div");
    widget2.className = style.widget;
    btnCollapse2.appendChild(iconCollapse);
    btnRecenter.appendChild(iconTarget);
    btnRecenter.appendChild(document.createTextNode("Recenter"));
    btnPause.appendChild(iconPause);
    collapsible2.appendChild(previewSelf.canvas);
    collapsible2.appendChild(rowButtons);
    previewSelf.canvas.style.transform = "scaleX(-1)";
    rowCollapser.appendChild(btnCollapse2);
    rowButtons.appendChild(btnRecenter);
    rowButtons.appendChild(btnPause);
    widget2.appendChild(rowCollapser);
    widget2.appendChild(collapsible2);
    sceneOverlay.root.appendChild(widget2);
    this.el.sceneEl.addEventListener("facetracking_start", () => {
      this.uncollapse();
    });
    Object.assign(this, { widget: widget2, sceneOverlay, previewSelf, collapsible: collapsible2, iconCollapse, iconPause, btnPause });
  },
  update: function() {
...
Object.fromEntries(blendShapeNames.map((name) => [name, doMirror(name)]));
Object.fromEntries(blendShapeNames.map((name) => [name, 0]));
window.blendShapeNames = blendShapeNames;
function injectDependency(baseComponent, dependentComponent) {
  var _a, _b;
  (_b = (_a = AFRAME.components[baseComponent]).dependencies) != null ? _b : _a.dependencies = [];
  AFRAME.components[baseComponent].dependencies.push(dependentComponent);
}
function isValidAvatar(avatarRootEl) {
  try {
...
AFRAME.registerComponent("facetracking-target", {
  ...
  init: function() {
    ...
    this.socket = lookup(`${"https://expressive-avatars-server.herokuapp.com"}/consumer`, {
      query: {
        type: isPrimary ? "primary" : "peer",
        token: isPrimary ? APP.store.state.credentials.token : void 0,
    ...
    this.el.object3D.traverse((obj) => {
      var _a;
      if (obj.isSkinnedMesh && ((_a = obj.morphTargetInfluences) == null ? void 0 : _a.length) > 0) {
    ...
    const onFace = ({ blendShapes, headOrientation }) => {
      ...
      if (blendShapes && headOrientation) {
        ...
        for (let blendShape in blendShapes) {
          ...
          this.eyes.right.rotation.set(-Math.PI / 2 + blendShapes["eyeLookDownRight"] * 0.5 - blendShapes["eyeLookUpRight"] * 0.5, 0, Math.PI - blendShapes["eyeLookOutRight"] + blendShapes["eyeLookOutLeft"]);
          this.eyes.left.rotation.copy(this.eyes.right.rotation);
        }
        (_a = this.headQuaternion) == null ? void 0 : _a.fromArray(headOrientation);
      }
    };
  ...
  tick: function() {
    var _a;
    this.headQuaternion = (_a = this.headQuaternion) != null ? _a : this.el.components["ik-controller"].headQuaternion;
  ...
  reset: function() {
    var _a;
    (_a = this.headQuaternion) == null ? void 0 : _a.identity();
    for (let skinnedMesh of this.skinnedMeshes) {
      for (let i2 = 0; i2 < skinnedMesh.morphTargetInfluences.length; ++i2) {
        skinnedMesh.morphTargetInfluences[i2] = 0;
      }
    }
  },
  remove: function() {
...
link.rel = "stylesheet";
link.type = "text/css";
link.href = "https://ios.aelatgt.net/lib/style.css";
